...
def collect_human_trajectory(env, device):
	...
	while not reset:
		...
		# 	print(env.sim.data.get_joint_qpos('r_gripper_r_finger_joint'), env.sim.data.get_joint_qpos('r_gripper_l_finger_joint'))
...
def gather_demonstrations_as_hdf5(directory, out_dir):
	...
	for ep_directory in os.listdir(directory):
		...
		ep_data_grp.create_dataset("states", data=np.array(states, dtype=np.float64))
		ep_data_grp.create_dataset("joint_velocities", data=np.array(joint_velocities, dtype=np.float64))
		ep_data_grp.create_dataset(
			"gripper_actuations", data=np.array(gripper_actuations, dtype=np.float64)
		)
		ep_data_grp.create_dataset("right_dpos", data=np.array(right_dpos, dtype=np.float64))
		ep_data_grp.create_dataset("right_dquat", data=np.array(right_dquat, dtype=np.float64))
		ep_data_grp.create_dataset("left_dpos", data=np.array(left_dpos, dtype=np.float64))
		ep_data_grp.create_dataset("left_dquat", data=np.array(left_dquat, dtype=np.float64))
...
def gather_demonstrations_as_hdf5_eef(directory, out_dir):
	"""
	Gathers the demonstrations saved in @directory into a
	single hdf5 file, and another directory that contains the
	raw model.xml files.
	...
	hdf5_path = os.path.join(out_dir, "demo_eef.hdf5")
	f = h5py.File(hdf5_path, "w")
	...
	for ep_directory in os.listdir(directory):
		...
		for state_file in sorted(glob(state_paths)):
			...
			states.extend(dic["states"])
			eef_states.extend(dic['eef_states'])
		...
		# write datasets for states and actions
		ep_data_grp.create_dataset("states", data=np.array(states, dtype=np.float64))
		ep_data_grp.create_dataset("eef_states", data=np.array(eef_states, dtype=np.float64))
		ep_data_grp.create_dataset("joint_velocities", data=np.array(joint_velocities, dtype=np.float64))
		ep_data_grp.create_dataset(
			"gripper_actuations", data=np.array(gripper_actuations, dtype=np.float64)
		)
...
if __name__ == "__main__":
	...
	env = DataCollectionWrapper(env, tmp_directory, collect_freq=collect_freq)
	...
	if True:
		...
		gather_demonstrations_as_hdf5_eef(tmp_directory, new_dir)
